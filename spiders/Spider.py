...
class Spider:
    ...
    def fetchData(self, url, parameters=None, retry=0):
        ...
        if self.opener is None:
            cnn = ('Connection', 'keep-alive')
            xm = ('X-MicrosoftAjax', 'Delta=true')
            self.opener = self.createOpener([config.USER_AGENT, cnn, xm])
            urllib2.install_opener(self.opener)
        try:
    ...
    def fetchData1(self, url, parameters=None, retry=0):
        """
        Fetch data from a url
        url='' Ex. http://www.example.com, https://www.example.com
        parameters={} Ex. {'user': 'user', 'pass': 'pass'}
        ...
        if self.opener is None:
            ...
            urllib2.install_opener(self.opener)
        self.opener.addheaders.append(('Referer', 'http://environmentclearance.nic.in/Search.aspx'))
        self.opener.addheaders.append(('Cookie', 'ASP.NET_SessionId=pddyz5armsrkcmelyushv255'))
        self.opener.addheaders.append(('Host', 'environmentclearance.nic.in'))
        cnn = ('Connection', 'keep-alive')
        self.opener.addheaders.append(cnn)
        try:
            if parameters is None:
                return self.opener.open(url, timeout=config.TIMEOUT).read()
            else:
